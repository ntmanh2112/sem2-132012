...
public class VacanciesDAO {
	...
	public static ArrayList<VacanciesModel> getAllVacancies(){
		...
		try {
			String sql = "SELECT Vacancy_ID,SecID,Designation_ID,No_Of_Vacancies,Status,cast(datepart(month,Vacancy_Date) as varchar)+'/'+ cast(datepart(day,Vacancy_Date) as varchar)+'/'+ cast(datepart(year,Vacancy_Date) as varchar)as'Vacancy_Date',Creator,Priority FROM Vacancies";
	...
	public static boolean updateVacancies(VacanciesModel model){
		Boolean kq = false;
		try {
			String sql = "UPDATE Vacancies SET SecID = ?, Designation_ID = ?, No_Of_Vacancies = ?, Status = ?, Vacancy_Date = ?, Creator = ?, Priority = ? WHERE Vacancy_ID = ?";
			PreparedStatement ps = DataUtil.getConnection().prepareStatement(sql);
			//ps.setString(1, model.getDep_ID());
			ps.setString(1, model.getSecID());
			ps.setString(2, model.getDesignation_ID());
			ps.setString(3, model.getNo_Of_Vacancies());
			ps.setString(4, model.getStatus());
			ps.setString(5, model.getVacancy_Date());
			ps.setString(6, model.getCreator());
			ps.setString(7, model.getPriority());
			ps.setString(8, model.getVacancy_ID());
			ps.executeUpdate();
			kq = true;
		} catch (Exception e) {
			// TODO Auto-generated catch block
	...
	public static ArrayList<VacanciesModel> searchVacancies(String Vacancy_ID, String SecID){
		ArrayList<VacanciesModel> listVacancies = new ArrayList<VacanciesModel>();
		try {
			String sql = "SELECT VACANCY_ID,SECID,Designation_ID,No_Of_Vacancies,Status,Vacancy_Date,Creator,Priority FROM VACANCIES WHERE Vacancy_ID LIKE '%" +Vacancy_ID+ "%' AND SecID LIKE '%" +SecID+ "%'";
			ResultSet rs = DataUtil.executeQuery(sql);
